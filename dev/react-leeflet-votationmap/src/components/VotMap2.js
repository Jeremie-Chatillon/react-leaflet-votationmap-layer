...
export default withLeaflet(class VotMap2 extends MapLayer {
  static propTypes = {
  ...
  updateHeatmapRadius(radius: number, blur: ?number): void {
    if (radius) {
      this._heatmap.setRadius(radius);
    }
  }
  ...
  fitBounds(): void {
    ...
    //console.info("fitBounds", lngs)
    const ne = { lng: max(lngs), lat: max(lats) };
    const sw = { lng: min(lngs), lat: min(lats) };
    console.info("fitBounds", ne, sw)
    if (shouldIgnoreLocation(ne) || shouldIgnoreLocation(sw)) {
      return;
    }

    console.info(L.latLngBounds(L.latLng(sw.lng, sw.lat), L.latLng(ne.lng, ne.lat)))

    this.props.leaflet.map.fitBounds(L.latLngBounds(L.latLng(sw.lng, sw.lat), L.latLng(ne.lng, ne.lat)));
  }
